...
#define WL_TRACE(x) do {if (dhd_msg_level & DHD_TRACE_VAL) printf x;} while (0)
#define WL_ASSOC(x)
#define WL_INFORM(x)
#define WL_WSEC(x)
#define WL_SCAN(x)
...
)
{
	...
	if (g_onoff == G_WLAN_SET_ON) {
		...
		irq_set_irq_wake(MSM_GPIO_TO_INT(dhd_oob_gpio_num), 0);		
...
)
{
	...
	irq_set_irq_wake(MSM_GPIO_TO_INT(dhd_oob_gpio_num), 1);		
...
/*porting,WIFI Module,hanshirong 66539,20101108 end-- */
	...
	if (strncmp(sub_cmd, "AP_CFG", 6)) {
		...
		if (strncmp(sub_cmd, "APSTA_CFG", 9)) {	
	   		    WL_ERROR(("ERROR: sub_cmd:%s != 'AP_CFG' or 'APSTA_CFG!\n", sub_cmd));
		    return -1;
	    }
		apsta_enable = TRUE;
	}
...
/* fix bug */
	...
	/* fix bug */
	p += snprintf(p, 256, "%s", mac_lst);
	p_iwrq->data.length = p - extra + 1;
...
)
{
	...
	if ( apsta_enable && ap_net_dev ) {
		printk("%s: stop the ap part of apsta concurrent\n", __FUNCTION__);
...
#endif /* #if  defined(CONFIG_WEXT_WPS2) */
#ifdef SOFTAP
#ifndef AP_ONLY
...
/*report sta leave msg to upper layer as deauth event received,*/
...
#ifdef SOFTAP
				if ( apsta_enable && ap_net_dev ) {
					printk("%s: schedule to restart the apsta ap part\n", __FUNCTION__);
	...
	{
		if (status != WLC_E_STATUS_SUCCESS) {
			printk("%s: WLC_E_SET_SSID, connect to AP failed, restart AP.\n", __FUNCTION__);
#ifdef SOFTAP
			if ( apsta_enable && ap_net_dev ) {
				printk("%s: schedule to restart the apsta ap part\n", __FUNCTION__);
...
int wl_iw_attach(struct net_device *dev, void * dhdp)
{
	...
	g_first_counter_scans = 0;
	g_iscan->scan_flag = 0;
#if defined(OEM_ANDROID) && defined(CONFIG_WEXT_WPS2)
	g_wps_probe_req_ie = NULL;
	g_wps_probe_req_ie_len = 0;
#endif /* #if defined(CONFIG_WEXT_WPS2) */
